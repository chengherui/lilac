...
//用户名、id互查
class Module_3 extends CI_Controller {
	...
	public function index() {
        ...
        if( $nickname == "木谦" || strpos($mods,'module_3') || strpos($mods,'module_3') === 0 ) {
            if($nickname && $mods) {
                $data['nickname'] = $nickname;
                $data['mods'] = $mods;
                $results = array();

                $sql = "select name, nickname, mods from yy_user";
                $this->load->database('tips');
            ...
            if($nickname && $mods) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    foreach ($nicknames as $i => $one) {
                        $name = trim($one);
                        if($name !== "") {
                            $sql = "select unit_id from lz_unit_taobao where visitor_id = '{$name}' limit 1";
                            $query_user = $this->db->query($sql);
                            $cnt = 0;
                            $cnt = $query_user->num_rows;
                            if($cnt == 1) {
